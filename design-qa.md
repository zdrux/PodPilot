...
- Visual hierarchy: the existing PodPilot shell now uses the selected Mission Control direction: compact dark surfaces, restrained cyan accents, thin ShadCN-style borders, compact controls, and a dedicated evidence rail.
- Timeline: completed operations are ordered chronologically and show tool name, status, start time, cluster, duration, a bounded request or command preview, and a working View action.
...
- Details view: the Timeline and Details tabs switch correctly. Details keeps collected evidence separate from model conclusions and links into the existing evidence dialog.
- Operation inspection: View opens a native dialog containing retained request/command, stdout, stderr, observations, limitations, error, status, cluster, and timing fields as available.
...
- Accessibility: activity tabs expose tab semantics and selection state; dialogs have labelled headings and close controls; the filter icon has an accessible explanation.
...
- The selected visual's persistent evidence rail, compact density, divider-led surfaces, cyan accent, status chips, and per-operation View actions are carried into the existing application.
...
- [x] Timeline and Details tabs exercised in the browser.
