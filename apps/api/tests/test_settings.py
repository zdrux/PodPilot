...
def test_incident_policy_is_editable_and_bounded() -> None:
    from podpilot_diagnostics.incident_policy import IncidentPolicy
    assert IncidentPolicy().coordinator_concurrency == 3
    assert IncidentPolicy().max_namespaces == 0
    assert IncidentPolicy().max_evidence_bytes == 16_777_216
    for invalid in ({"max_rounds": 0}, {"max_collection_bytes": -1}, {"page_size": 501}, {"specialist_concurrency": 9}):
        with pytest.raises(ValidationError):
            IncidentPolicy(**invalid)
